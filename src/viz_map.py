...
def _route_color_for_slope(slope):
    """평균 경사 그라데이션 (인터랙티브·정적 일관)."""
    import pandas as pd_local
    if pd_local.isna(slope):
        return "#888"
...
def add_bus_routes(m):
    """통학버스 노선 — 평균 경사 그라데이션. 기본 OFF."""
    from src.config import ROUTES_SHP

    try:
        from src.route_slope import load_routes
        routes_5179 = load_routes()
    except Exception as e:
        print(f"   [경고] 노선 로드 실패: {e}")
        return None

    rs_csv = DATA_PROCESSED / "routes_with_slope.csv"
    if not rs_csv.exists():
        print(f"   [경고] routes_with_slope.csv 없음, 노선 레이어 스킵")
        return None
    rs = pd.read_csv(rs_csv, encoding="utf-8-sig")
    slope_by_id = dict(zip(rs["Id"], rs["slope_mean"]))
    max_by_id   = dict(zip(rs["Id"], rs["slope_max"]))

    routes_4326 = routes_5179.to_crs("EPSG:4326")
    routes_4326["length_m"] = routes_5179.geometry.length.values

    fg = folium.FeatureGroup(
        name=f"🛣️ 통학버스 노선 ({len(routes_4326)}개)",
        overlay=True, show=False
    )

    for _, r in routes_4326.iterrows():
        rid = r.get("Id")
        slope = slope_by_id.get(rid)
        smax = max_by_id.get(rid)
        color = _route_color_for_slope(slope)
        purpose = r.get("목적", "")
        chasu = r.get("차수", "")
        length_km = float(r["length_m"]) / 1000.0
        slope_str = f"{slope:.1f}°" if pd.notna(slope) else "-"
        max_str   = f"{smax:.1f}°"  if pd.notna(smax)  else "-"
        popup_html = (
            f'<div style="font-family:Malgun Gothic,sans-serif;font-size:12.5px;">'
            f'<b style="font-size:14px;color:#1A237E;">{r["short"]}</b><br>'
            f'노선 길이: <b>{length_km:.2f} km</b><br>'
            f'평균 경사: <b style="color:{color};">{slope_str}</b> · '
            f'최대 경사: {max_str}<br>'
            f'목적: {purpose} · 차수: {chasu}'
            f'</div>'
        )

        if r.geometry.geom_type == "MultiLineString":
...
def add_bus_stops(m):
    """통학버스 정류장 — 작은 흰 원. 기본 OFF."""
    from src.config import STOPS_SHP
    try:
        g = gpd.read_file(STOPS_SHP, encoding="cp949")
    except Exception as e:
        print(f"   [경고] 정류장 SHP 로드 실패: {e}")
        return None
    if g.crs.to_epsg() != 4326:
        g = g.to_crs("EPSG:4326")

    fg = folium.FeatureGroup(
        name=f"🚏 통학버스 정류장 ({len(g)}개)",
        overlay=True, show=False
    )
    for _, r in g.iterrows():
        ...
        popup_html = (
            f'<div style="font-family:Malgun Gothic,sans-serif;font-size:12.5px;">'
            f'<b style="font-size:13px;color:#1A237E;">{탑승}</b><br>'
            f'소속: {소속}<br>'
            f'경유 순서: {경유} · {목적} {차수}차'
            f'</div>'
        )
        folium.CircleMarker(
            ...
            radius=4, fill=True, fillColor="white", fillOpacity=1.0,
            color="#1A237E", weight=1.5, opacity=0.9,
...
def add_route_overlap_polys(m):
    """공동활용 후보 중첩 영역 polygon (Phase 3). 기본 OFF."""
    import json as _json
    geojson_path = DATA_GEOJSON / "노선중첩영역.geojson"
    if not geojson_path.exists():
        return None
    # UTF-8로 직접 로드 (folium 내부의 cp949 디코드 회피)
    with open(geojson_path, encoding="utf-8") as f:
        geo_data = _json.load(f)

    fg = folium.FeatureGroup(
        name="🔄 노선 중첩 영역", overlay=True, show=False
    )
    ...
    folium.GeoJson(
        ...
        popup=folium.GeoJsonPopup(
            fields=["school_A", "school_B", "rate_max"],
            aliases=["학교 A", "학교 B", "중첩률 max"],
            localize=False,
            labels=True,
            style="background:white;font-family:Malgun Gothic;font-size:12px;",
        ),
        tooltip=folium.GeoJsonTooltip(
            fields=["school_A", "school_B"],
            aliases=["", "+"],
            sticky=True,
        ),
    ).add_to(fg)
...
def build_map(schools_df, output_filename="대전_외부환경분석_도시개발.html",
    ...
    fg_bus_routes = add_bus_routes(m)
    fg_bus_stops = add_bus_stops(m)
    fg_overlap = add_route_overlap_polys(m)
    ...
    _add_custom_panel(
        ...
        bus_routes_fg=fg_bus_routes,
        bus_stops_fg=fg_bus_stops,
        overlap_fg=fg_overlap,
...
def _add_custom_panel(m, base_tiles, admin_fgs, school_fgs,
                       ...
                       bus_routes_fg=None, bus_stops_fg=None,
                       overlap_fg=None):
    ...
    fg_routes = bus_routes_fg
    fg_stops = bus_stops_fg
    fg_overlap = overlap_fg
...
"""))
    ...
    if fg_routes: off_layers.append(n(fg_routes))
    if fg_stops: off_layers.append(n(fg_stops))
    if fg_overlap: off_layers.append(n(fg_overlap))
    ...
    # 통학버스 그룹 — 노선·정류장·중첩 체크박스 (있을 때만)
    bus_routes_chk = (
        f'<label><input type="checkbox" onchange="lpLayer(this,\'{n(fg_routes)}\')"> '
        f'🛣️ 노선 (평균 경사 색상)</label>'
        if fg_routes is not None else ''
    )
    bus_stops_chk = (
        f'<label><input type="checkbox" onchange="lpLayer(this,\'{n(fg_stops)}\')"> '
        f'🚏 정류장</label>'
        if fg_stops is not None else ''
    )
    overlap_chk = (
        f'<label><input type="checkbox" onchange="lpLayer(this,\'{n(fg_overlap)}\')"> '
        f'🔄 노선 중첩 (공동활용 후보)</label>'
        if fg_overlap is not None else ''
    )
...
<div id="layer-panel">
  ...
  <div class="lp-group">
    <div class="lp-header" onclick="lpToggle(this)">🚌 통학버스 운영 <span class="arrow">▼</span></div>
    <div class="lp-body">
      <label><input type="checkbox" checked onchange="lpLayer(this,'{n(fg_bus14)}')"> 🚌 운영 학교 ({n_bus14}교: 운영 12 + 예정·명목 각 1)</label>
      {bus_routes_chk}
      {bus_stops_chk}
      {overlap_chk}
    </div>
